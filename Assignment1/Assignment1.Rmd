...
**Q3 Were there any expression values that could not be mapped to current HUGO symbols?** NO. The dataset uses HUGO notation. 

**Q6 How did you handle replicates in this data?** There are no replicates in this dataset.  
...
## GO term mapping
Mapping to the GO terms to observe the overall genes included in the data set.
```{r GO mapping, message=FALSE, warning=FALSE, error=FALSE}
mapped_terms <- select(org.Hs.eg.db, keys=b3$refGene, keytype="SYMBOL", columns= c("GENENAME", "ONTOLOGY" ,"PATH"))
...
### Box plot
At this stage, there is no clear separation between the overall expression data between the three sample groups: young, old, AD diseased. 
...
par(mfrow = c(2, 2))
plotMA(log2(b3_filtered30[, c(ad_cols[3], old_cols[3])]), ylab="M - ratio log expression", 
       main="AD Diseased sample 3 vs Old sample 3")
       abline(h = 0, col = "blue3", lwd = 2, lty = "dashed")
plotMA(log2(b3_filtered30[, c(ad_cols[3], young_cols[3])]), ylab="M - ratio log expression", 
       main="AD Diseased sample 3 vs Young sample 3")
       abline(h = 0, col = "blue3", lwd = 2, lty = "dashed")
plotMA(log2(b3_filtered30[, c(young_cols[3], old_cols[3])]), ylab="M - ratio log expression", 
...
There is not significant change in the density plot after normalization.
```{r Density, message=FALSE, error=FALSE, warning=FALSE, fig.height= 8, fig.width=10}
par(mfrow = c(1, 2))
...
From the MDS Plot, three clusters are observed. The green one representing the AD population clusters in the center, while the old population clusters at the left hand side of the graph. Red, representing young population in the sample, scatter across x a-xis, but does not have a y value high than 0.5. 

```{r MDS plot, message=FALSE, error=FALSE, warning=FALSE}
plotMDS(d, labels=rownames(samples[1:30,]),
col = c("darkgreen","blue", "red")[factor(samples[1:30,]$disease_type)])
```
### BVC plot # does not work now
```{r}
model_design <- model.matrix(~samples$sample_number
+ samples$disease_type +0)
...
RMarkdown Cookbood: https://bookdown.org/yihui/rmarkdown-cookbook/figure-size.html
